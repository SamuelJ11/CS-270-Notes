...
### Basic Principles
    ...
    • For example, if E is declared as int E[N], and we wish to evaluate E[i], where the address of E is stored in register %rdx and 'i' is stored in %rcx, then movl(%rdx, %rcx, 4), %eax will perform the address computation xₑ + 4i, read that memory location, and store the result in %eax.

### Pointer Arithmetic 

    • The array dereference A[i] is identical to the expression *(A + i)

    • Below are some examples involving 'E' from before and their corresponding assembly code implementation
...
### Nested Arrays
    ...
    • To access elements of multi-dimensional arrays, the compiler generates code to compute the offset of the desired element and then uses one of the mov instructions with the start of the array as the base address and the (possibly scaled) offset as an index:
...
### Fixed Array size
    ...
    • The C compiler can make many optimizations for code operating on multi-dimensional arrays of fixed size.
    ...
    • Suppose we wish to compute ONE ELEMENT in the product matrix of two fix_matrices (N x N) together, where A and B are these matrices, and C is the product of both A and B:
    ...
    • The C code in 'fix_matrix.c' is implemented by the following assembly code, where:
...
### Variable Size arrays
    ...
    • The dynamic version must use a multiplication instruction to scale i by n, rather than a series of shifts and adds. On some processors, this multiplication can incur a significant performance penalty, but it is unavoidable in this case.

    * See textbook page 292 for C code and assembly that multiplies two VLA's and computes their corresponding entry.
...
### Structures
    ...
    • Suppose we have the following struct below, where variable 'r' of type struct rec* is stored in register %rdi:
        ...
        int *p      // 8 bytes, offset 48
        };

        * r is a pointer to rec (the address of the struct)
        * i and j are field names inside the structure

        - this can be modeled by the following assembly code:
            ...
            to that memory location: this overwrites the old value of j with the value of i, effectively copying the value of r->i to
            to r->j.
    ...
    • Finally we consider the last example:
...
### Unions
    ...
    • If you know that only one of several fields will ever be used at a time, you don’t need separate memory for each field, so a union
        ...
        - consider a binary tree, a data structure made up of nodes where each node can have at most two children
            ...
            -- internal nodes have at least one child (left, right, or both), and do not store actual data, but 
    ...
    • With this encoding, however, there is no way for the compiler to determine whether a given node is a leaf or an internal node:

        - A common method is to introduce an enumerated type defining the different possible choices for the union, and then create a
            structure containing a tag field and the union:

            typedef enum { N_LEAF, N_INTERNAL } nodetype_t;         // This creates a new type called nodetype_t and can only take two values: N_LEAF or N_INTERNAL.
            struct node_t {
                nodetype_t type;                                    // 'type' is a nodetype_t variable that stores whether the node is a leaf (N_LEAF) or an internal node (N_INTERNAL).
                union {                                             // this union is where the 'real' contents of the node live.
                    struct {
                        struct node_t *left;
                        struct node_t *right;
                    } internal;                                     // field name of the internal struct
                    double data[2];
                } info;                                             // field name of the union
            };

### Data Alignment

    • Many computer systems place restrictions on the allowable addresses for primitive data types, requiring that the address for some objects 
        must be a multiple of some value K (typically 2, 4, or 8).
    ...
    • For code involving structures, the compiler may need to insert gaps in the field allocation to ensure that each structure element satisfies its alignment requirement. The structure will then have a required alignment for its starting address:
        ...
        - the struct above without padding has the following memory allocation:
        ...
        - since the largest field is double, the compiler needs an 8-byte alignment, so we would insert padding after the char
        ...
        - now we have satisfied the following constraints:

            char c: 1-byte alignment     (c starts at a multiple of 1)
            int i: 4-byte alignment      (i starts at a multiple of 4)                    
            double d: 8-byte alignment   (d starts at a multiple of 8)

        - it is important to note that any pointer to struct 'example' must also have an 8-byte alignment
    
    • Similarly, the compiler  may need to add padding to the end of the structure so that each element in an array of structures will satisfy its alignment requirement.

    • Can we do better (i.e., use less memory)?

        - declaring the largest element first can SOMETIMES reduce padding and save memory.
        
        - below is the same example struct with largest-first ordering:
        ...
        - in this example, largest-first ordering did not actually help us, but see the lecture slides for an example where it does!